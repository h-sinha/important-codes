...
int st[N*6], qt[N*6];
void make_tree(int s, int e,int index) {
	if(s == e-1) {
		st[index] = baseArray[s];
		return;
	}
	int c1 = (index<<1), c2 = c1 | 1, m = (s+e)>>1;
	make_tree(s, m,c1);
	make_tree(m, e,c2);
	st[index] = st[c1] > st[c2] ? st[c1] : st[c2];
}
...
void query_tree(int index, int s, int e, int l, int r) {
	if(s >= r || e <= l) {
		qt[index] = -1;
		return;
	}
	...
	int c1 = (index<<1), c2 = c1 | 1, m = (s+e)>>1;
	query_tree(c1, s, m, l, r);
	query_tree(c2, m, e, l, r);
	qt[index] = qt[c1] > qt[c2] ? qt[c1] : qt[c2];
}
